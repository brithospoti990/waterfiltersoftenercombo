...
- [ ] **Verify affiliate destinations.** The `/go/*` redirects currently point to `springwellwater.com/?oid=N&affid=40`. Replace each destination with the exact product-page tracking URL from your Everflow dashboard (format: `springwellwater.com/product/...?oid=N&affid=40`) — same as your other sites.
- [ ] Replace `static/author-kelly.svg` with a real headshot (`author-kelly.jpg`, ~400x400, <60 KB) and update the two `<img src="/author-kelly.svg">` references in `content/index.html` and `content/about.html`.
